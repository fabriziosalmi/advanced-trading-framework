...
class TradingApp:
            ...
            if st.button("📊 Generate Signals"):
                if self.initialized and selected_tickers:
                    with st.spinner("Generating signals..."):
                        signals = asyncio.run(self.generate_signals(selected_tickers))
    ...
    def render_monitoring_tab(self):
        ...
        if 'system' in health_status and 'checks' in health_status['system']:
            ...
            if checks_df:
                st.dataframe(pd.DataFrame(checks_df), use_container_width=True)
        ...
        if 'portfolio_history' in result and result['portfolio_history']:
            ...
            if history_data:
                ...
                if PLOTLY_AVAILABLE:
                    ...
                    st.line_chart(chart_df['Portfolio Value'], use_container_width=True)
    ...
    def calculate_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Calculate technical indicators for a given price dataframe with caching."""
        try:
    ...
    def get_stock_data(self, symbol: str, period: str = "6mo") -> pd.DataFrame:
        """Get stock data for technical analysis with caching."""

        try:
            import yfinance as yf
        except ImportError:
            st.error("yfinance is required to fetch stock data. Install it via pip install yfinance")
            return pd.DataFrame()   
    ...
    def render_enhanced_ticker_selector(self, key: str, default_selected: List[str] = None,
        ...
        if show_sectors:
            st.markdown("#### 🎯 Quick Sector Selection")

            # Sector preset buttons
            cols = st.columns(4)
            for i, (sector, tickers) in enumerate(tickers_by_sector.items()):
                with cols[i % 4]:
                    if st.button(f"📊 {sector[:8]}", key=f"sector_{sector}_{key}",
                                help=f"Select all {sector} stocks"):
                        if allow_multiple:
                            # Add sector tickers to current selection
                            current = set(st.session_state[session_key])
                            current.update(tickers)
                            st.session_state[session_key] = list(current)
                        else:
                            st.session_state[session_key] = tickers[:1]  # First ticker only
                        st.rerun()
    ...
    def _calculate_risk_metrics(self) -> Dict[str, float]:
        """Calculate comprehensive risk metrics for the portfolio with caching."""
        try:
            if not self.portfolio or not hasattr(self.portfolio, 'trade_history'):
                return _calculate_risk_metrics_cached([], [])

            # Get portfolio value history and trade history
            value_history = self.portfolio.get_portfolio_value_history()
            closed_trades = [trade for trade in self.portfolio.trade_history if trade.get('action') == 'CLOSE']

            return _calculate_risk_metrics_cached(value_history, closed_trades)
...
def _calculate_risk_metrics_cached(value_history: List[Dict], closed_trades: List[Dict]) -> Dict[str, float]:
    """Calculate comprehensive risk metrics for the portfolio with caching.

    Args:
        value_history: List of portfolio value history entries
    ...
    try:
        ...
        # Log error if logger is available, otherwise just return defaults
        return {
            'sharpe_ratio': 0.0,
            'max_drawdown_pct': 0.0,
            'volatility_annualized': 0.0,
...
def main():
    ...
    st.sidebar.write(f"Session initialized: {st.session_state.get('app_initialized', False)}")
    st.sidebar.write(f"App initialized: {trading_app.initialized}")
    st.sidebar.write(f"Portfolio exists: {trading_app.portfolio is not None}")
    st.sidebar.write(f"Broker exists: {trading_app.broker is not None}")
    st.sidebar.write(f"Strategies count: {len(trading_app.strategies) if hasattr(trading_app, 'strategies') else 0}")
    st.sidebar.write(f"Signals in session: {len(st.session_state.get('signals_data', []))}")
