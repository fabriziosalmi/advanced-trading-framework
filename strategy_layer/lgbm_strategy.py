...
class MLLGBMStrategy(Strategy):
    ...
    def __init__(self, confidence_threshold: float = 0.6):
        ...
        if LGBMClassifier is None:
            raise ImportError("scikit-learn and lightgbm are required for MLLGBMStrategy")
        ...
        # Data cache with weak references for memory efficiency
        self.data_cache: weakref.WeakValueDictionary = weakref.WeakValueDictionary()
    ...
    def cleanup(self):
        ...
        self.logger.info(f"MLLGBMStrategy initialized with confidence threshold: {self.confidence_threshold}")
    ...
    def _fetch_data(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Note: To shift to intraday trading, we would need to change interval to '5m' and acquire a specialized, 
            # high-quality data source, as free sources like yfinance have limitations on historical intraday data. 
            # Furthermore, the feature engineering (e.g., SMA(50)) and target definitions would need to be completely 
            # recalibrated for the new, much higher frequency.
            try:
            ...
            self.logger.info(f"Fetched {len(data)} days of data for {ticker}")
    ...
    def _calculate_technical_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_columns = [
            # Core features only - minimal set for baseline
            'rsi_14',           # Momentum indicator
            'atr_14',           # Volatility indicator  
            'sma_50',           # Needed for slope and price_to_sma calculations
            'sma_50_slope',     # Trend indicator
            'macd_hist',        # Momentum/trend indicator
            'volume_ratio_20',  # Volume indicator
            'spy_returns',      # Market context
            'price_to_sma_50',  # Value/mean reversion
            ...
            'relative_momentum_5',  # 5-day relative momentum
            'relative_momentum_20', # 20-day relative momentum
            # Momentum decay/acceleration features
            'momentum_decay_5',         # 5-day momentum decay
            'momentum_decay_10',        # 10-day momentum decay
            'momentum_acceleration_5_10', # Acceleration from 5d to 10d momentum
            'momentum_acceleration_10_20' # Acceleration from 10d to 20d momentum
        ]
        ...
        for i in range(len(df)):
            ...
            # RSI (14-day)
            if i >= 14:
                delta = current_data['close'].diff().tail(14)
                gain = (delta.where(delta > 0, 0)).mean()
                loss = (-delta.where(delta < 0, 0)).mean()
                rs = gain / loss if loss != 0 else 100
                rsi = 100 - (100 / (1 + rs))
                result_df.iloc[i, result_df.columns.get_loc('rsi_14')] = rsi

            # ATR (14-day)
            if i >= 13:
                high_low = current_data['high'].tail(14) - current_data['low'].tail(14)
                high_close = (current_data['high'].tail(14) - current_data['close'].shift(1).tail(14)).abs()
                low_close = (current_data['low'].tail(14) - current_data['close'].shift(1).tail(14)).abs()
                true_range = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1)
                atr = true_range.mean()
                result_df.iloc[i, result_df.columns.get_loc('atr_14')] = atr

            # SMA 50 and slope
            if i >= 49:
                result_df.iloc[i, result_df.columns.get_loc('sma_50')] = current_data['close'].tail(50).mean()
                # Calculate SMA slope (change over last 5 days)
                if i >= 54:
                    sma_50_now = result_df['sma_50'].iloc[i]
                    sma_50_prev = result_df['sma_50'].iloc[i-5]
                    result_df.iloc[i, result_df.columns.get_loc('sma_50_slope')] = (sma_50_now - sma_50_prev) / sma_50_prev if sma_50_prev != 0 else 0

            # Price to SMA 50
            if i >= 49 and not pd.isna(result_df.iloc[i]['sma_50']):
                result_df.iloc[i, result_df.columns.get_loc('price_to_sma_50')] = (current_data['close'].iloc[-1] / result_df.iloc[i]['sma_50']) - 1
            ...
            # Volume Ratio (20-day)
            if i >= 19:
                result_df.iloc[i, result_df.columns.get_loc('volume_ratio_20')] = current_data['volume'].iloc[-1] / current_data['volume'].tail(20).mean()

            # === MOMENTUM DECAY AND ACCELERATION FEATURES ===
            # Calculate momentum decay (how much momentum has weakened over time)
            if i >= 10:  # Need 10 days for decay calculations
                # 5-day momentum decay: current 5-day momentum vs 5-day momentum 5 days ago
                current_momentum_5d = current_data['close'].pct_change(5).iloc[-1]
                past_momentum_5d = current_data['close'].pct_change(5).iloc[-6] if i >= 10 else current_momentum_5d
                momentum_decay_5 = current_momentum_5d - past_momentum_5d
                result_df.iloc[i, result_df.columns.get_loc('momentum_decay_5')] = momentum_decay_5

            if i >= 20:  # Need 20 days for 10-day decay
                # 10-day momentum decay: current 10-day momentum vs 10-day momentum 10 days ago
                current_momentum_10d = current_data['close'].pct_change(10).iloc[-1]
                past_momentum_10d = current_data['close'].pct_change(10).iloc[-11] if i >= 20 else current_momentum_10d
                momentum_decay_10 = current_momentum_10d - past_momentum_10d
                result_df.iloc[i, result_df.columns.get_loc('momentum_decay_10')] = momentum_decay_10

                # Momentum acceleration: rate of change of momentum
                momentum_5d = current_data['close'].pct_change(5).iloc[-1]
                momentum_10d = current_data['close'].pct_change(10).iloc[-1]
                momentum_acceleration_5_10 = momentum_10d - momentum_5d  # Difference between longer and shorter momentum
                result_df.iloc[i, result_df.columns.get_loc('momentum_acceleration_5_10')] = momentum_acceleration_5_10

            if i >= 30:  # Need 30 days for 20-day momentum comparison
                momentum_10d = current_data['close'].pct_change(10).iloc[-1]
                momentum_20d = current_data['close'].pct_change(20).iloc[-1]
                momentum_acceleration_10_20 = momentum_20d - momentum_10d  # Difference between 20d and 10d momentum
                result_df.iloc[i, result_df.columns.get_loc('momentum_acceleration_10_20')] = momentum_acceleration_10_20

            # === REGIME DETECTION FEATURES ===
            # Bull/Bear/Neutral market classification based on trend and momentum
            if i >= 49 and not pd.isna(result_df.iloc[i]['sma_50_slope']):
                price_to_sma = result_df.iloc[i]['price_to_sma_50']
                slope = result_df.iloc[i]['sma_50_slope']
            ...
            if i >= 13 and not pd.isna(result_df.iloc[i]['atr_14']):
                # Compare current ATR to its 20-day moving average
                if i >= 33:  # Need enough data for ATR MA
                    atr_ma = result_df['atr_14'].iloc[i-20:i].mean()
                    current_atr = result_df.iloc[i]['atr_14']
        ...
        try:
            ...
            if spy_data is not None:
                ...
                for i in range(len(result_df)):
                    if i >= 1:  # Need at least 1 day for returns
                        ...
                        if len(spy_row) > 0:
                            spy_close = spy_row['close'].iloc[-1]
                            spy_prev_close = spy_row['close'].iloc[-2] if len(spy_row) > 1 else spy_close
                            ...
                            stock_return = current_data['close'].pct_change(1).iloc[-1] if i >= 1 else 0
                            relative_strength = stock_return - spy_return
                            result_df.iloc[i, result_df.columns.get_loc('relative_strength')] = relative_strength
                            
                            # Calculate relative momentum (5-day and 20-day)
                            if i >= 5:
                                stock_returns_5d = current_data['close'].pct_change(5).iloc[-1] if i >= 5 else 0
                                spy_returns_5d = (spy_close - spy_row['close'].iloc[-6]) / spy_row['close'].iloc[-6] if len(spy_row) > 5 else 0
                                result_df.iloc[i, result_df.columns.get_loc('relative_momentum_5')] = stock_returns_5d - spy_returns_5d
                            
                            if i >= 20:
                                stock_returns_20d = current_data['close'].pct_change(20).iloc[-1] if i >= 20 else 0
                                spy_returns_20d = (spy_close - spy_row['close'].iloc[-21]) / spy_row['close'].iloc[-21] if len(spy_row) > 20 else 0
                                result_df.iloc[i, result_df.columns.get_loc('relative_momentum_20')] = stock_returns_20d - spy_returns_20d
        except Exception as e:
            ...
            for i in range(len(result_df)):
                if i >= 1:
                    ...
                    if i >= 5:
                        result_df.iloc[i, result_df.columns.get_loc('relative_momentum_5')] = result_df['close'].pct_change(5).iloc[i] * 0.3
                    if i >= 20:
                        result_df.iloc[i, result_df.columns.get_loc('relative_momentum_20')] = result_df['close'].pct_change(20).iloc[i] * 0.3
    ...
    def _prepare_features_and_target(self, data: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
        ...
        # Select feature columns - using only core features
        core_features = [
            'rsi_14',           # Momentum indicator
            'atr_14',           # Volatility indicator  
            'sma_50_slope',     # Trend indicator
            ...
            'relative_momentum_5',  # Short-term relative momentum
            'relative_momentum_20', # Long-term relative momentum
            'momentum_decay_5',     # Short-term momentum decay
            'momentum_decay_10',    # Medium-term momentum decay
            'momentum_acceleration_5_10', # Momentum acceleration 5d->10d
            'momentum_acceleration_10_20' # Momentum acceleration 10d->20d
        ]
    ...
    def _train_model_sync(self, ticker: str, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            # Only save if model meets minimum performance criteria
            ...
            if evaluation_metrics['f1_score'] > min_f1_threshold:
    ...
    async def generate_signals(self, data: Dict[str, pd.DataFrame], portfolio: Dict[str, Any],
            ...
            try:
                ...
                if ticker not in self.models:
                    if not self._load_model_for_ticker(ticker):
                        if not self._train_model_for_ticker(ticker):
                            self.logger.warning(f"Could not train or load model for {ticker}")
                            continue
                
                # Get the data for this ticker from the passed data parameter
                ...
                ticker_data_with_indicators = self._calculate_technical_indicators(ticker_data.copy())
                
                # Debug: Log available columns
                self.logger.info(f"Available columns for {ticker}: {list(ticker_data_with_indicators.columns) if ticker_data_with_indicators is not None else 'None'}")
                self.logger.info(f"Data shape for {ticker}: {ticker_data_with_indicators.shape if ticker_data_with_indicators is not None else 'None'}")
                
                if ticker_data_with_indicators is None or ticker_data_with_indicators.empty:
                    self.logger.warning(f"No processed data available for {ticker}")
                    continue
                
                # Update data cache with the processed data for feature extraction
                self.data_cache[ticker] = ticker_data_with_indicators
                self.last_data_update[ticker] = datetime.now()
                
                # Get latest features directly from the processed data
                try:
                    latest_data = ticker_data_with_indicators[self.feature_columns].iloc[-1:].fillna(0)
                    if len(latest_data) == 0:
                        continue
                    features = latest_data
                except KeyError as e:
                    self.logger.error(f"Missing features for {ticker}: {e}")
                    continue
                ...
                # Get current price for signal - use processed data with normalized columns
                current_price = None
                try:
                    self.logger.info(f"Trying to access 'close' column for {ticker}")
                    current_price = float(ticker_data_with_indicators['close'].iloc[-1])
                    self.logger.info(f"Successfully got current_price for {ticker}: {current_price}")
                except KeyError as e:
                    self.logger.error(f"'close' column not found for {ticker}. Available columns: {list(ticker_data_with_indicators.columns)}")
                    raise e
                
                # Detect market regime - use processed data
                regime = self._detect_market_regime(ticker_data_with_indicators)
                
                # Adjust confidence based on regime
                adjusted_confidence = self._adjust_confidence_for_regime(confidence, regime)
                
                # Calculate risk metrics - use processed data
                risk_metrics = self._calculate_risk_metrics(ticker_data_with_indicators, regime)
                
                # Generate signal based on prediction and adjusted confidence
                if prediction == 1 and adjusted_confidence >= self.confidence_threshold:
                    # BUY signal
                    stop_loss_pct, take_profit_pct = self._calculate_stop_levels(ticker_data_with_indicators, 'buy', regime, risk_metrics)
                    ...
                    signal = create_buy_signal(
                        ...
                        take_profit=current_price * (1 + take_profit_pct) if current_price else None,
                        reasoning=self._generate_signal_reasoning(ticker, 'buy', adjusted_confidence, regime, risk_metrics, probabilities),
                        model_type="RandomForest",
                        prediction_prob=adjusted_confidence
                    )
                    signals.append(signal)
                    self.add_signal_to_history(signal)
                    
                elif prediction == 0 and adjusted_confidence >= self.confidence_threshold:
                    # SELL signal
                    stop_loss_pct, take_profit_pct = self._calculate_stop_levels(ticker_data_with_indicators, 'sell', regime, risk_metrics)
                    ...
                    signal = create_sell_signal(
                        ...
                        stop_loss=current_price * (1 + stop_loss_pct) if current_price else None,
                        take_profit=current_price * (1 - take_profit_pct) if current_price else None,
                        reasoning=self._generate_signal_reasoning(ticker, 'sell', adjusted_confidence, regime, risk_metrics, probabilities),
                        model_type="RandomForest",
                        prediction_prob=adjusted_confidence
                    )
                    signals.append(signal)
                ...
                self.logger.error(f"Failed to generate signal for {ticker}: {str(e)}")
        
        self.logger.info(f"Generated {len(signals)} signals for {len(tickers)} tickers")
        
        # Convert TradingSignal objects to dictionaries expected by backtesting engine
        signal_dicts = []
        for signal in signals:
            signal_dict = {
                'symbol': signal.ticker,
                'side': signal.action.lower(),  # 'buy' or 'sell'
                'quantity': getattr(signal, 'position_size', 1),  # Default to 1 if not set
    ...
    async def _train_fold(self, ticker: str, train_data: pd.DataFrame, test_data: pd.DataFrame, fold_num: int):
        ...
        try:
            ...
            # Ensure we have feature names for LGBM
            feature_names = X_train.columns.tolist()
            
            # Quick training without hyperparameter optimization for speed
            model = LGBMClassifier(n_estimators=100, random_state=42, n_jobs=-1, verbosity=-1)
            model.fit(X_train_scaled, y_train, feature_name=feature_names)
    ...
    async def _train_final_model_for_deployment(self, ticker: str, final_train_data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                self.executor, self._train_model_sync, ticker, final_train_data
            )
            return result
